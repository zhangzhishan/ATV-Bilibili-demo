...
class LoginViewController: UIViewController {
    ...
    private func styleLoginUI() {
        ...
        configuration.title = "刷新二维码"
        configuration.contentInsets = NSDirectionalEdgeInsets(top: 20, leading: 40, bottom: 20, trailing: 40)
        configuration.baseBackgroundColor = BLVisualTheme.accent
        configuration.baseForegroundColor = .black
        configuration.cornerStyle = .capsule
        regenerateButton.configuration = configuration
        regenerateButton.titleLabel?.font = UIFont.systemFont(ofSize: 34, weight: .bold)

        titleLabel.textColor = BLVisualTheme.textPrimary
        titleLabel.font = UIFont.systemFont(ofSize: 64, weight: .bold)
        guideLabel.textColor = BLVisualTheme.textSecondary
    }
